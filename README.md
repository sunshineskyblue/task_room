...
# 作成目的
プログラミングスクールでの課題をベースにポートフォリオとして機能を拡張致しました。
ポートフォリオを作ることが一つの目的となる中で、自分としては、
作成を通じてrailsの知識を増やし、スキルをさらに高めることを命題としていました。
課題であった宿泊サイトは、予約、お知らせ、レビューと言った連動が求められるCRUD機能を始め、
ホストゲスト間でのインターアクティブな操作が求められることから、
より高度なDB設計、あるいはMVCにおけるロジックの役割性、メンテナンス性が重要となり、
railsを学習する上で最適な制作対象であると考えました。
また、制作期間を約2ヶ月と自分の中で定め、限られた時間の中で、
...
# 今後の課題
アプリケーションとして実際に公開するにはまだ多くの課題があり、
railsの勉強にもつながるため、今後も制作を継続する予定です。
